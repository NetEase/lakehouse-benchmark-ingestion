...
//        configuration.setString(TaskManagerOptions.MANAGED_MEMORY_SIZE.key(), "1024");
        context.getEnv().getCheckpointConfig().setCheckpointStorage(
                new FileSystemCheckpointStorage("file:///Users/hzmajin/Documents/workspace/hudi_test"));
        ...
        final SingleOutputStreamOperator<Void> process = context.getEnv()
            .fromSource(source, WatermarkStrategy.noWatermarks(), "mysql").uid("mysql").setParallelism(1)
            .process(new RowDataVoidProcessFunction(getConverters(pathAndTable))).uid("split stream").name("split stream").setParallelism(12);
