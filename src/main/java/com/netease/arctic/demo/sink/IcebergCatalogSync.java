...
public class IcebergCatalogSync extends BaseEduardCatalog {
    @Override
    public void createTable(Catalog catalog, String dbName, List<Tuple2<ObjectPath, ResolvedCatalogTable>> pathAndTable) throws DatabaseAlreadyExistException {
        Catalog iceberg = catalog;
        if (!iceberg.databaseExists(dbName)) {
            try {
        ...
        pathAndTable.forEach(e -> {
            try {
                ...
                if (iceberg.tableExists(objectPath) || objectPath.getObjectName().equals("history")) {
                    iceberg.dropTable(objectPath, true);
                }
                iceberg.createTable(objectPath, new ResolvedCatalogTable(e.f1.copy(options), e.f1.getResolvedSchema()), false);
